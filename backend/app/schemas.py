...
class SiteBase(BaseModel):
    ...
    user_context: Optional[str] = Field(None, description="User-defined context for this site")
...
class SiteResponse(SiteBase, TimestampSchema):
    ...
    id: int
    pages_count: Optional[int] = Field(None, description="Number of pages associated with this site")
...
class PageBase(BaseModel):
    ...
    url: str = Field(..., min_length=1, max_length=2048, description="Full URL of the page")
    title: Optional[str] = Field(None, max_length=500, description="Page title")
    page_summary: Optional[str] = Field(None, description="AI-generated summary of the page")
    user_context: Optional[str] = Field(None, description="User-defined context for this page")
...
class PageResponse(PageBase, TimestampSchema):
    ...
    sections_count: Optional[int] = Field(None, description="Number of sections extracted from this page")
...
class NoteBase(BaseModel):
    ...
    position_x: Optional[int] = Field(None, description="X coordinate for note positioning")
    position_y: Optional[int] = Field(None, description="Y coordinate for note positioning")
    anchor_data: Optional[Dict[str, Any]] = Field(None, description="DOM anchoring data as JSON")
    is_active: bool = Field(True, description="Whether the note is active")
    server_link_id: Optional[str] = Field(None, max_length=100, description="External reference ID")
...
class NoteResponse(NoteBase, TimestampSchema):
    ...
    id: int
    page_id: int
    artifacts_count: Optional[int] = Field(None, description="Number of artifacts generated for this note")
...
class LLMProviderBase(BaseModel):
    ...
    provider_type: str = Field(..., min_length=1, max_length=50, description="Provider type (e.g., 'claude', 'gemini')")
    api_endpoint: Optional[str] = Field(None, max_length=255, description="API endpoint URL")
    model_name: str = Field(..., min_length=1, max_length=100, description="Model name")
    max_tokens: int = Field(4096, gt=0, le=100000, description="Maximum tokens for generation")
    ...
    configuration: Optional[Dict[str, Any]] = Field(None, description="Additional provider configuration")
...
class LLMProviderResponse(LLMProviderBase, TimestampSchema):
    ...
    id: int
    artifacts_count: Optional[int] = Field(None, description="Number of artifacts generated by this provider")
...
class NoteArtifactBase(BaseModel):
    ...
    artifact_type: str = Field(..., min_length=1, max_length=50, description="Type of artifact (e.g., 'summary', 'expansion')")
...
class PageSectionBase(BaseModel):
    ...
    section_type: str = Field(..., min_length=1, max_length=50, description="Type of section (e.g., 'header', 'paragraph')")
    content: str = Field(..., min_length=1, description="Section content")
    selector: Optional[str] = Field(None, max_length=500, description="CSS selector for the section")
    xpath: Optional[str] = Field(None, max_length=500, description="XPath for the section")
    position_in_page: int = Field(..., ge=0, description="Position of section within the page")
...
class ArtifactGenerationRequest(BaseModel):
    ...
    artifact_type: str = Field(..., min_length=1, max_length=50, description="Type of artifact to generate")
    llm_provider_id: int = Field(..., description="ID of the LLM provider to use")
    custom_prompt: Optional[str] = Field(None, description="Custom prompt to use instead of default template")
...
class PageSummarizationRequest(BaseModel):
    ...
    llm_provider_id: int = Field(..., description="ID of the LLM provider to use")
    summary_type: str = Field("general", description="Type of summary (e.g., 'general', 'technical', 'key_points')")
    custom_prompt: Optional[str] = Field(None, description="Custom prompt for summarization")
...
class BulkNoteCreate(BaseModel):
    ...
    notes: List[NoteCreate] = Field(..., min_items=1, max_items=100, description="List of notes to create")


class BulkNoteResponse(BaseModel):
    """Schema for bulk note creation response."""
    created_notes: List[NoteResponse] = Field(..., description="Successfully created notes")
